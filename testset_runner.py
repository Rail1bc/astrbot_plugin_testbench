...
任务驱动，运行记录保存在内存中、可轮询、可找回、可取消：

- sequential：逐条消息发出 → 等待该步全部会话完成 → 再发下一条（上下文连续）。
- batch：所有消息立即连续发出（重叠），再逐个收集结果。
...
# 单步超时（秒）：一步的全部会话未在此时限内完成即标记该步失败并中止（sequential）
# 或继续收其余步（batch），防止悬挂 pipeline 拖死整个测试集。
...
class TestsetRunner:
    ...
    def start_run(self, testset: dict, sessions: list[dict], mode: str) -> str:
    ...
    async def _drive(self, run_id: str) -> None:
        ...
        try:
            if run["mode"] == "batch":
                await self._drive_batch(run)
            else:
                await self._drive_sequential(run)
    ...
    async def _drive_sequential(self, run: dict) -> None:
    ...
    async def _drive_batch(self, run: dict) -> None:
