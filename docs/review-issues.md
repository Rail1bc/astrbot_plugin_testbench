...
- **修复**：pytest.yml 双矩阵锁定 `astrbot==4.24.1`（最低支持版）+ `4.26.6`（已实测版，`fail-fast: false`），pytest 系同步锁定（pytest 9.1.1 / pytest-asyncio 1.4.0）；3 条直接依赖内部模块的用例标 `framework_internal` 标记（pyproject 注册），最低版矩阵 `-m "not framework_internal"` 跳过。
