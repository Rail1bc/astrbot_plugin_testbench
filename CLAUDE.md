...
- `identity_list.js`：「身份与群聊」rail 第三视图。`createIdentityList(env)`（env 注入 refreshGroups/showRunStatus）由 app.js 创建，不 import app.js。左侧卡片内以 **tab 拆分**「身份」/「群聊」两个列表（`switchIdentityTab` 按 dataset.tab 分发、pane 互斥显隐，身份膨胀后群聊列表不被长身份列表挤走）：`#identity-list` 身份列表（名称 + sender_id/sender_name 徽标 + 编辑 / 删除，`openIdentityForm` 走 modal 表单——sender 留空回退名称、更新传空串而非 null 让后端重置）；`#chat-group-list` 虚拟群聊列表（名称 + N 成员徽标 + 选中高亮 + 编辑 / 删除，条目整体点击 → 右侧打开编辑视图）。**创建群聊只填名称**（成员多选移出弹窗——成员多时窗口放不下，`openCreateChatGroup` 提交 `createChatGroup({ name })`，成功后自动选中新群聊）；成员管理在右侧「群聊编辑」视图完成（`renderChatGroupView` 按 `state.selectedChatGroupId` 渲染）：搜索身份池（`renderSearchResults` 按名称/发送者ID/昵称子串过滤、排除已在群成员、空关键字显示全部可加入成员）点「＋ 加入」`addMember`、成员行「✕」`removeMember`、头部「保存名称」`saveChatGroupName`、悬空成员引用（身份已删除）保留占位可移除。`syncBroadcastSenders()` 同步群发栏 `#run-sender` 身份选择器选项（「各会话自身身份」为默认）。增删改后 `refreshGroups()` 回刷组弹窗选项。
...
> 由 GitHub Actions 自动把关——push 到 dev 触发 `pytest.yml`（171 个测试 +
...
- `tests/test_frontend.py`：前端脚本静态检查（31 个），零依赖，任何环境可运行。
