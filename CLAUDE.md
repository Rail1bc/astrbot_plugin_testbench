...
> 由 GitHub Actions 自动把关——push 到 dev 触发 `pytest.yml`（150 个测试 +
...
- `tests/test_frontend.py`：前端脚本静态检查（22 个），零依赖，任何环境可运行。
