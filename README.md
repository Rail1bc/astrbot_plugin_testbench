...
> [!NOTE]
> 当前版本 v0.4.0，支持测试组模型、并行会话查看、逐会话独立刷新、群发重叠发送（agent 处理中可再次群发）、面板在途消息实时状态、群发会话实时统计与历史 JSON 编辑 / 重新生成；并支持群聊消息类型与自动@、测试身份与虚拟群聊、群消息流视图与唤醒状态反馈。
...
> **开发流程**：本地不跑测试，修改直接提交推送到 `dev` 分支，由 GitHub Actions 自动把关——push 到 dev 触发 `pytest.yml`（177 个测试 + 前端 JS 语法检查）与 `ruff-format.yml`；dev 验证通过后合并到 `main`，metadata.yaml 变更即触发 release.yml 自动发版。以下本地命令仅在主动排查时使用。
