...
TestsetsAPI / IdentitiesAPI / EventsAPI / ConfRouteMixin），由 main.py 的
VirtualSessionPlugin 继承装配——handler 保持为 Star 实例上的 bound method，
_ROUTES 的 getattr 解析与测试的 ``plugin.<handler>`` 调用方式均不变。
_ROUTES 路由表集中于此，main.py 注册时引用。
"""
