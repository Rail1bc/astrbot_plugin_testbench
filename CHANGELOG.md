...
- **README 明确适用场景与受众定位**：新增「适用场景」——小白用户（主要，对比提示词 / 配置 / 模型的回复差异，告别凭感觉调参）、技术用户（进阶，测试集做极端场景稳定性压测）、**不推荐插件开发者调试自己的插件**（虚拟链路不经过真实平台适配器，与真实链路存在差异）；口号与价值支柱改为「直观对比」导向，功能详解首节同步加注。
