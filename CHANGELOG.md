...
- **README 细化新手引导**：修复「三步开始」流程断链（新建测试组默认 0 会话，须先经 ✎ 编辑设置「会话数量」才能打开会话）、修正「使用流程」中已移除的「添加」按钮描述（新增会话走编辑弹窗「会话数量」）；配置项表格补全「会话数量 / 消息类型」，并新增「对比不同提示词 / 配置 / 模型」用法提示——在 AstrBot 框架本体建多个配置档案，测试组内每个会话绑定不同档案，群发同一句话并排对比差异。
- **README 开发者章节改为贡献者流程**：移除「直接推送到 dev」的维护者流程表述，统一为 fork + 向 dev 提 PR（CI 在 PR 上自动把关）；本地测试命令路径改为克隆仓库后的通用形式。

## [v1.0.0] - 2026-08-07

### 🎉 里程碑（Milestone）

- **1.0.0 正式发布**：自 v0.4.5 以来无功能变更，本次升版为「功能完备（设计文档 v1 + v2 全部落地）+ 测试门禁（pytest.yml 320 个测试函数 + 前端 JS 检查 + ruff）+ 发布管道跑通（多轮发版）」的**稳定契约声明**——此后 `data/virtual_session/*.json` 数据格式与 Web API 按兼容性对待，变更需提供迁移。
- **已知限制（不影响核心测试场景，均有检测兜底）**：
  - **虚拟会话无法捕获主动推送（proactive send）**：插件在固定时间经 cron 主动向用户发送的问候等出站消息，走 `Context.send_message` 出站路径、不经虚拟会话入站通道，运行结果不可见、不可断言。完整方案需 AstrBot 核心增加「主动消息发送」钩子（跨仓库 PR）；当前以 cron 探测 + 晚到检测兜底（运行级警告）。
  - **评审材料在捕获快照缺失时按配置重建被测 Agent 系统提示词**：评审输入中的【被测 Agent 系统提示词】块在快照为空时由 `eval/persona.py` 从会话配置档案重新解析人格拼出，非真实传给被测 LLM 的装饰后版本（可能被其他插件 / hook 改写）；捕获成功时即为真值。

### 📝 文档与工程化（Docs & Chores）

- **README 按价值主张风格重写，面向一般用户组织语言**：参照同类成熟插件 README 风格重构——口号 + 三大价值支柱表格开头、「三步开始」极简上手、功能详解按使用场景组织概念级语言、开发细节收拢到独立章节；移除文件目录树（开发者可在 CLAUDE.md 查看）。
- **README 明确适用场景与受众定位**：新增「适用场景」——快速上手（主要，对比提示词 / 配置 / 模型的回复差异，告别凭感觉调参）、进阶使用（测试集做极端场景稳定性压测）、**不推荐插件开发者调试自己的插件**（虚拟链路不经过真实平台适配器，与真实链路存在差异）；口号与价值支柱改为「直观对比」导向，功能详解首节同步加注。
- **插件元数据简介改为面向使用场景的直白描述**：以「轻松对比调提示词」场景开头，点明并发对比不同提示词 / 插件配置 / 模型下的回复差异，进阶可用测试集做极端场景稳定性压测。
- **README 靠前位置增加醒目警告**：提示虚拟会话走真实处理管道的副作用——绑定工具档案会真实执行计算机工具、异步补发与定时任务可能命中虚拟会话、删除会话不清理第三方插件以会话为键写入的长期数据（死数据遗留）。
- **贡献指南 / PR 模板 / CI 同步**：CONTRIBUTING Python 3.12+、无第三方依赖、dev 分支约定；PR 模板依赖声明对齐；pytest.yml js-check 补齐 render_markdown.js 模块（16 个）。
